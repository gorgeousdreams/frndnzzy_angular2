"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var app_events_service_1 = require('./app.events.service');
var http_1 = require('@angular/http');
var DashboardEventsComponent = (function () {
    function DashboardEventsComponent(eventsService, http) {
        this.eventsService = eventsService;
        this.http = http;
    }
    DashboardEventsComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.eventsService.readEvents("51")
            .subscribe(function (result) {
            _this.result = result.data;
            _this.tresult = result.data;
            console.log(_this.result);
        }, function (error) { return _this.error = error; });
    };
    DashboardEventsComponent.prototype.Search = function (title) {
        title = title.trim();
        this.result = this.tresult.filter(function (t) { return t.title.toLowerCase().startsWith(title.toLowerCase()); });
    };
    DashboardEventsComponent = __decorate([
        core_1.Component({
            selector: 'dashboard-events',
            templateUrl: '../app/events/dashboard.events.html',
        }), 
        __metadata('design:paramtypes', [app_events_service_1.EventsService, http_1.Http])
    ], DashboardEventsComponent);
    return DashboardEventsComponent;
}());
exports.DashboardEventsComponent = DashboardEventsComponent;
//# sourceMappingURL=dashboard.events.js.map